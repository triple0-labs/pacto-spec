...
- **Internal refactor (no behavior change):** Wave 3 of light DDD layering — extracted the `pacto normalize` pipeline (resolve plans root → discover plans → for each: validate, normalize, optionally write → aggregate counters) into a dedicated `internal/app/normalizeplans` use-case package with a typed `ErrInvalid` sentinel for unresolvable roots. `internal/command/normalize` shrank to a thin Cobra wrapper that only owns flag parsing, format validation, JSON encoding, and table rendering. Coverage on the new package: 77.8%.
- **Internal refactor (no behavior change):** Wave 3 of light DDD layering — extracted the `pacto exec` pipeline (validate state/slug, resolve plan doc, mark next or specified task done, append note/blocker/evidence under localized headings, refresh "Last Modified") into a dedicated `internal/app/execplan` use-case package with a typed `ErrInvalid` sentinel for validation problems. `internal/command/execmd` shrank to a thin Cobra wrapper that only owns flag parsing and pretty output. Coverage on the new package: 77.8%.
- **Internal refactor (no behavior change):** Wave 3 of light DDD layering — extracted the `pacto doctor` drift-analysis pipeline (validate flags → resolve root/tools → analyze → summarize → compute exit code) into a dedicated `internal/app/doctor` use-case package with a typed `ErrInvalid` sentinel for validation problems. `internal/command/doctor` now delegates and only owns table/JSON rendering, the soft drift warning, and `--verbose` trace. Coverage on the new package: 69.0%.
- **Internal refactor (no behavior change):** Wave 3 of light DDD layering — extracted the `pacto init` workspace bootstrap (state directories, README/PACTO templates, context dir, config.yaml, prd.md, optional managed AGENTS.md block, optional per-tool artifacts) into a dedicated `internal/app/initws` use-case package with a typed `ErrInvalid` sentinel for filesystem clashes. The Cobra command in `internal/command/initcmd` keeps the interactive wizard, validation, dry-run, install confirmation prompt, and summary printing; the actual bootstrap pipeline is now a single `initws.Apply` call (with `initws.ApplyInstall` available for the deferred install branch). Coverage on the new package: 78.4%.
- **Internal refactor (no behavior change):** Wave 3 of light DDD layering — extracted the `pacto install` (and the artifact-refresh half of `pacto update`) pipeline (resolve project root → resolve tool list / auto-detect → generate per-tool artifacts → aggregate stats) into a dedicated `internal/app/installtools` use-case package with a typed `ErrInvalid` sentinel for validation problems. `internal/command/install` now delegates to the use case for that branch and only owns flag parsing, printing, and self-update. Coverage on the new package: 77.1%.
- **Internal refactor (no behavior change):** Wave 3 of light DDD layering — extracted the `pacto new` plan-scaffold pipeline (validate → resolve plan root → ensure / validate workspace → write README + spec/design/tasks templates) into a dedicated `internal/app/newplan` use-case package with a typed `ErrInvalid` sentinel for validation problems. `internal/command/newcmd` is now a thin Cobra wrapper handling flags, `--lang` override, and pretty output. New direct unit tests cover the use case at 84.1%.
