...
func TestRunInitCreatesContextWorkspace(t *testing.T) {
	root := t.TempDir()
	if code := RunInit([]string{"--root", root, "--no-interactive", "--no-install"}); code != 0 {
		t.Fatalf("RunInit returned %d", code)
	}

	assertPathExists(t, filepath.Join(root, ".pacto", "context", "README.md"))
	assertPathExists(t, filepath.Join(root, ".pacto", "context", "domains"))
}
...
func TestRunNewUsesWorkspaceLanguageForGeneratedPlanDocs(t *testing.T) {
	...
	if !strings.Contains(specText, "## Dominios Afectados") || !strings.Contains(specText, "- <dominio>") {
		t.Fatalf("expected spanish domains section in spec, got %q", specText)
	}
