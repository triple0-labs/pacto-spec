...
func Workflows() []WorkflowSpec {
	return []WorkflowSpec{
		...
		},
		{
			...
			FallbackAction: "If intent is unclear, ask follow-up questions and create `prd.md` with explicit assumptions/open questions before proceeding.",
			Implemented:    true,
		},
		{
			WorkflowID: "install",
		...
		},
		{
			...
			Title:      "Pacto Exec",
			Summary:    "Execute plan tasks and register execution evidence in plan artifacts.",
			Command:    "pacto exec <state> <slug> [--root <path>] [--step <phase.task>] [--note <text>] [--blocker <text>] [--evidence <claim>] [--dry-run]",
			WhenToUse:  "Use after moving a plan to `current` to advance tasks while keeping execution evidence in plan documents.",
			RequiredInputs: []string{
			...
			FallbackAction: "Use `pacto status` to inspect blockers or task state if execution updates cannot be applied.",
			Implemented:    true,
		},
	}
}
