...
func Workflows() []WorkflowSpec {
	return []WorkflowSpec{
		...
		},
		{
			...
			Command:    "pacto exec <state> <slug> [--root <path>] [--step <task-id>] [--note <text>] [--blocker <text>] [--evidence <claim>] [--dry-run]",
			...
			OptionalInputs: []string{
				"`--root <path>` to target a specific project root.",
				"`--step <task-id>` to complete a specific task (for example, `T3`).",
